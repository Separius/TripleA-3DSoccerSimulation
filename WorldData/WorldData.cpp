...
{
    float dist = 9999;
    std::vector <int> oppUids = mr_found_opponents();
...
WorldData::getNearstOppToLine(Line line)
{
    float dist = 9999;
    int id = -1;
    std::vector <int> opponentIds = mr_found_opponents();
...
WorldData::getNearstOppToPoint(Point point)
{
    float dist = 9999;
    int id = -1;
    std::vector<int> oppIds = mr_found_opponents();
...
WorldData::basedCenter(vector_t v)
{
    ...
    std::cout << "me(true) : " << me(true) << std::endl;
    vector_t newVector;
...
void WorldData::setPositions()
{
    int they = mr_found_opponents().size();
    std::vector<int> Opps;

    Opps = mr_found_opponents();
    for (int i = 0; i < they; i++)
    {
        oppPositions[i] = basedCenter(opponent(Opps[i]));
    }

    //there is no need to do this each cycle they are only needed once a half (of the play time)
    oppPole1 = basedCenter(opgoal_pole1());
    oppPole2 = basedCenter(opgoal_pole2());
    ourPole1 = basedCenter(mygoal_pole1());
    ourPole2 = basedCenter(mygoal_pole2());

    std::vector<int> teammates = mr_found_teammates();
    for (int i = 0; i < teammates.size(); i++)
    {
        teammatePos[i] = basedCenter(teammate(teammates[i]));
    }
}

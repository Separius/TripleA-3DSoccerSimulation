...
void Pass::bestTeammateSelector()
{
    ...
    for (int i = 0; i < tmms.size(); i++)
    {
        ...
        {
            if (i == 0)
                thisTeammate = pWorldData->teammatePos[1];
            if (i == 1)
                thisTeammate = pWorldData->teammatePos[2];
            if (i == 2)
                thisTeammate = pWorldData->teammatePos[3];
            if (i == 3)
                thisTeammate = pWorldData->teammatePos[4];

            if (ballPos.getDistance(thisTeammate) > passReachableDist || ballPos.getDistance(thisTeammate) < 60)
            {
                thisValue = 0;
            }
            else
